...
## 2025-05-16 - Accessible Selection States
**Learning:** In highly visual interfaces with custom selection grids (like occupations or categories), standard visual cues (borders/colors) are insufficient for screen readers. Using `aria-pressed` on buttons provides clear, semantic state feedback.
**Action:** Always implement `aria-pressed` and an ARIA live region for dynamic feedback when refactoring custom selection components.
