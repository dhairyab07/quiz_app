...
function selectOccupation(event, occupation) {
  ...
  const target = event.currentTarget || event.target.closest("button");
  ...
  question.options.forEach((option, index) => {
    ...
    button.innerHTML = `
          <span class="flex items-center justify-between w-full">
              ...
              <span class="hidden sm:inline-block px-2 py-0.5 rounded border border-white/10 text-[10px] text-white/30 font-bold ml-2">
                ${index + 1}
              </span>
          </span>
      `;
